...
{ 
 ...
 if (!SD.begin(SD_ChipSelectPin)) // returns 1 if the card is present
 {
  ...
  return;
 } else { Serial.println("SD Passed");}

 tmrpcm.setVolume(5); //
 tmrpcm.play("creep.wav"); //the sound file "song" will play each time the arduino powers up, or is reset
                          //try to provide the file name with extension
                          
    Serial.println("Playing deadinsidetest");
...
{  
  while(digitalRead(pp)==0 || digitalRead(next)==0 || digitalRead(prev)==0)
  {
    if(digitalRead(pp)==0)
    {
      ...
      delay(200);
      Serial.println("Pressed Pause/Play");
    }
...
void song (void)
{
  if(temp==1)
  {
    tmrpcm.play("creep.wav");  
    Serial.println(" Playing Safe And Sound");
  }
  else if(temp==2)
  {
    tmrpcm.play("di_t.wav");  
    Serial.println("Playing dead inside (test)");
  }
  else if(temp==3)
  {
    tmrpcm.play("ubf.wav");  
    Serial.println("Playing upbeat funk");
  }
  else if(temp==4)
  {
    tmrpcm.play("ml.wav");  
    Serial.println("Playing moody loop");
  }
  else if(temp==5)
  {
    tmrpcm.play("sts.wav");  
